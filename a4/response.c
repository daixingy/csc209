...
void image_filter_response(int fd, const ReqData *reqData) {
    ...
    for(int i =0;i<MAX_QUERY_PARAMS;i++){
        if(reqData->params[i].name && strcmp(reqData->params[i].name,"filter")==0){
    ...
    for(int i =0;i<MAX_QUERY_PARAMS;i++){
        if(reqData->params[i].name && strcmp(reqData->params[i].name,"image")==0){
            if(strchr(reqData->params[i].value,'/') == NULL){
                value = i;
                count++;
                break;
    ...
    int exist_f,execute_f;
    char path_f[strlen(FILTER_DIR)+strlen(reqData->params[key].value)+1];
    strncpy(path_f,FILTER_DIR,strlen(FILTER_DIR)+1);
    strncat(path_f,reqData->params[key].value,strlen(reqData->params[key].value)+1);
    exist_f = access(path_f,F_OK);
    execute_f = access(path_f,X_OK);

    int exist_i,read_i;
    char path_i[strlen(IMAGE_DIR)+strlen(reqData->params[value].value)+1];
    strncpy(path_i,IMAGE_DIR,strlen(IMAGE_DIR)+1);
    strncat(path_i,reqData->params[value].value,strlen(reqData->params[value].value)+1);
    exist_i = access(path_i,F_OK);
    read_i = access(path_i,R_OK);

    if(count==2 && exist_f == 0 && exist_i == 0){
        if(execute_f == 0 && read_i == 0){
            write_image_response_header(fd);
            int n_image = open(path_i,O_RDONLY);
            if(dup2(n_image,fileno(stdin))==-1){
                perror("dup2");
                exit(1);
            }
            ...
            execl(path_f, path_f, NULL);

            
        }else{
            const char *message2 = "Error occured while executing filters";
            internal_server_error_response(fd, message2);
        }

    }else{
        const char *message = "This is a bad request";
        bad_request_response(fd, message);
    }
}
